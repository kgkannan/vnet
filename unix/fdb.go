...
func (m *FdbMain) ipFibRoute(c cli.Commander, w cli.Writer, in *cli.Input) (err error) {
	...
	for !in.End() {
		...
		case in.Parse("help"):
			...
			fmt.Fprintf(w, "<add|del> nh<ip> len<plen> intf <xeth> mac <hw-addr> where intf is xeth1,..\n")
			fmt.Fprintf(w, "<punt> prefix<ip> len<plen> ns <ns> intf <xeth> mac <hw-addr> where intf=xeth1, ns=netns of egress intf\n")
			return
